...
var catalogs = map[Locale]map[string]string{
	LocaleZhCN: {
		...
		"custom.fcitx_theme.desc":                  "安装内置的 Fcitx5 主题，并优先通过 DBus 设置当前主题。",
		"custom.fcitx_theme.hint":                  "选择一个主题，程序会先安装主题，再优先通过 DBus 设置它。",
	...
	LocaleEn: {
		...
		"custom.fcitx_theme.desc":                  "Install bundled Fcitx5 themes and prefer DBus when setting the active theme.",
		"custom.fcitx_theme.hint":                  "Choose one theme. The app installs it first, then tries DBus before falling back to config files.",
